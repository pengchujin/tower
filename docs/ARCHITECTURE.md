...
- Quantumult X：server_local、policy 和 filter_local。QX 不支持尾随 `no-resolve`；生成器会将其移除，并在标准/严格保护（或来源规则明确要求不解析）时，于 `final` 前写入同策略的 `host-keyword, .`。
...
- Quantumult X 仅在资源本身就是可用的 `filter_remote` 语法时远程引用。原生 filter 的第三列是策略名，远程引用时由 `force-policy` 覆盖；带 Clash/Surge `no-resolve` 的资源不属于合法 QX 语法，必须回落本地转换并由生成器换成 `host-keyword, .` 兜底。ACL4SSR 中其他不兼容的经典 Clash 规则同样本地转换。
