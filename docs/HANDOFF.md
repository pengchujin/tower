...
- **1.0.8 (46)** 发布准备：新版四页使用引导、设置内重看入口、AI 平台机器人示例；优化剪贴板异步读取与页面切换更新范围；修复地图地区节点收起时订阅流量信息错位。全量 1011 项 XCTest（3 跳过、0 失败）及 49 项 Swift Testing 通过，833 项界面文案本地化提取通过；内置规则为最新且 77 个远程产物回读通过。实体 iPhone 已覆盖安装并启动 1.0.8 (46) 优化 Debug。5 项引导、地图收起及剪贴板关键交互回归通过。正式 Release 归档上传状态待发布后确认。
