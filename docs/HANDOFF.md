...
## 1.0.16（54）发布准备（2026-09-11）

- 汇总首页刷新浮层、全界面 13 项动画以及管理页关闭搜索闪烁修正；用户要求发布 GitHub Mac 包，并将 iOS 与 Mac 构建上传 App Store Connect，商店版本由用户自行发布。
- 版本号与构建号已统一为 1.0.16（54）。发布前 iOS 全量 1,188 项中 1,182 通过、6 跳过、零失败；此前 8 项定向 UI 流程通过，动画开发包已覆盖安装手机供用户体验。归档、上传与 Mac 分发结果待本节更新。
