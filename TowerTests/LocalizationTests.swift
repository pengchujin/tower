...
final class LocalizationTests: XCTestCase {
    ...
    func testEnglishExportExplanationsAreNaturalAndSpecific() throws {
        ...
        let expected = [
            ...
            "塔台会通过 %@ 的 URL Scheme 打开客户端。配置只在这台 iPhone 的 127.0.0.1 临时地址保留 3 分钟，不会上传；需要更新时回到塔台再次导入。":
                "Tower opens %@ using its URL scheme. The profile stays on this iPhone at 127.0.0.1 for 3 minutes and is never uploaded. Return to Tower to import updates.",
